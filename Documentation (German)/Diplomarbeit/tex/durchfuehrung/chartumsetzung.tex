...
Nach dem anfänglichen Erstellen eines \inline{Chart}s und einer \inline{Series} muss die neue \inline{Series} zunächst so konfiguriert werden, dass es nach einem echten Candlestick-Chart aussieht. Dies funktioniert so:
...
//Zooming der Y-Achse durch den User ermöglichen und
...
//Zooming der X-Achse durch den User ermöglichen und
//Scrollbalken definieren.
...
Beim Zooming geschieht etwas sehr ähnliches, lediglich muss hier eine andere Variable der \inline{ChartArea} beeinflusst werden. Für den Zoom der X-Achse muss das Datum 100 Bars vor dem letzten und das des letzten Bars eingestellt werden. Dies wird allerdings aufgrund der Datentypenfreiheit dieser Angabe als \inline{double}-Wert verlangt und muss deshalb mit Hilfe der Methode \inline{ToOADate()} in ein Datum im OLE Automation Format umgerechnet werden. Weiters müssen die Grenzen der y-Achse erneut mit einem Aufschlag von 5\% auf die zuvor gesuchten Werte definiert werden. Im Code sieht das wie folgt aus:
...
Nun, da das Aussehen der \inline{Series} definiert ist, müssen nur noch die Daten, die vom Benutzer übergeben wurden, korrekt in diese übernommen werden. Dazu muss die gesamte Preisliste in einer Schleife durchgegangen werden, um aus jedem Bar-Tupel den Open-, den High-, den Low- und den Close-Wert auslesen und in die \inline{Series} speichern zu können. Dies muss für jeden Bar-Tupel so durchgeführt werden, wobei auch die Reihenfolge sehr wichtig ist:

\begin{lstlisting}[label=Hinzufügen der Werte eines Bars zur Series,caption=Hinzufügen der Werte eines Bars zur Series]
...
Im Laufe des Projekts wurde entschieden, dass die Entscheidungen des Algorithmus im Chart in Form von \inline{ArrowAnnotation}s dargestellt werden sollen. Diese sind einfache Pfeile, die an einen bestimmten Bar in der \inline{Series} angehängt werden können. Farblich wurden für die Pfeile der Verkaufssignale -1, -2 und -3 drei unterschiedliche Rottöne gewählt. Entsprechend wurden für die Kaufsignale 1, 2 und 3 drei unterschiedliche Grüntöne gewählt. Außerdem zeigen die roten Pfeile nach unten und die grünen nach oben. Neutralpfeile, die das Signal 0 darstellen, sind die weißer Farbe gehalten. Da es sich bei Neutralpfeile immer um eine Neutralisierung der zuvor eingegangenen Position sind, zeigen die Neutralpfeile bei der Neutralisierung einer Short-Position (-1, -2 oder -3) nach oben, da gekauft werden muss, und bei der Neutralisierung einer Long-Position (1, 2 oder 3) nach unten, da verkauft werden muss. Die \inline{Annotation}s werden zu einer eigenen Eigenschaft des Charts, nämlich \inline{Annotations} hinzugefügt und die Berechnung des Aussehens der einzelnen Pfeile erfolgt direkt beim Hinzufügen jedes einzelnen Bars, sofern an diesem ein Signalwechsel des Algorithmus vorliegt. Die Berechnung der Signale muss daher klarerweise vor dem Aufbau des Charts erfolgen. Im Code sind \inline{ArrowAnnotation}s für einen roten Pfeil wie folgt realisiert. Für die anderen Pfeile müssen nur die jeweiligen Eigenschaften geändert werden:
...
Grundsätzlich können in der \gls{bts} auf dem Settings-Tab unter "`Chart"' neue Indikatoren hinzugefügt werden, die bei der Erstellung des Charts nach einer Berechnung automatisch in neue \inline{Series} eingezeichnet und so dargestellt werden. Intern wird dazu in der \gls{bts} eine Liste aus \inline{StackPanel}s gespeichert. Wenn ein neuer Indikator hinzugefügt wird, wird in dieser Liste ein neues \inline{StackPanel} erzeugt. Dieses beinhaltet, je nach Indikator, den es repräsentieren soll, unterschiedliche \gls{wpf}-\gls{gui}-Komponenten, die alle notwendigen Informationen speichern können. Es wurde die Unterstützung vier verschiedener Indikatoren (für finanzwirtschaftliche Erklärung siehe Abschnitt \ref{Indikatoren} "`Indikatoren"') implementiert:
...
Wurden diese Indikatoren festgelegt, wird bei folgenden Berechnungen nach der einfachen bisherigen Charterstellung auch noch die bereits erwähnte \inline{StackPanel}-Liste durchsucht. Dabei werden die Informationen jedes Indikators anhand seines Namens ausgelesen, es wird eine neue \inline{Series} erstellt und der Berechnung mittels \inline{FinancialFormula} übergeben. Dies funktioniert wie in \ref{finfor} "`Finanzielle Berechnungen"' beschrieben. Die Werte des Indikators werden automatisch in die \inline{Series} geschrieben und diese kann problemlos gezeichnet werden.\\
\\
Beim zuletzt erwähnten Indikator, dem \gls{macd}, gilt es allerdings noch eine weitere Problematik zu beachten. Als einziger der implementierten Indikatoren fluktuiert der \gls{macd} nämlich nicht um die Aktien-Preisdaten herum, sondern um die X-Achse. Da dies in der selben Zeichnung nicht wirklich schön aussehen würde, wird zur Darstellung des \gls{macd} nun eine extra \inline{ChartArea} erzeugt, die sich unter der Haupt-Area befindet. Eine solche zweite \inline{ChartArea} lässt sich grundsätzlich auch recht einfach auf dieselbe Art und Weise wie die erste erzeugen, nur dass sie noch auf die erste \inline{ChartArea} ausgerichtet werden muss. Hier wurde eine Größe von 70\% des gesamten Charts für die Candlestick-Area und 30\% für die \gls{macd}-Area festgelegt. Dies funktioniert wie folgt:
