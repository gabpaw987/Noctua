...
Entwickelt wurde der \gls{cci}, wie der Name bereits andeutet, für Commodities, also Handelsgüter, kann heutzutage aber ebenso gut für Wertpapiere und Derivate verwendet werden. Im Prinzip ist er ein Trendfolger, der sowohl Momentum, als auch Preis berücksichtigt.
Der \gls{cci} zeigt den Handelspreis im Verhältnis zu einem \gls{ma} über eine bestimmte Zeitdauer und wurde vom Erfinder Donald R. Lambert mithilfe eines Divisors, der auf der Standardabweichung basiert, normalisiert. Daher bewegt sich der \gls{cci} zwischen den Werten -100 und +100. Wenn sich der Preis stärker verändert als es im Zeitraum des \gls{ma} passiert ist, bewegt sich der \gls{cci} über diese Grenzen hinweg. Vom Erfinder wurde eine long-Position bei Werten über +100 und eine short-Position unter -100 empfohlen. Heutzutage benutzen viele Trader diese Grenzen in umgekehrter Manier als Support- und Resistance-Level, wodurch sie bevor sich ein Trend umkehrt auf einen möglichen Umschwung hingewiesen werden. Solange sich der \gls{cci} zwischen den beiden signifikanten Werten bewegt spricht man von einem trendlosen Markt.\\
	Berechnet wird der CCI, indem vom typischen Preis (s. \ref{typical_price}) der \gls{sma} abgezogen wird und die Differenz, wie in Formel \ref{cci} beschrieben, durch die Standardabweichung dividiert wird. Der Gewichtungsfaktor von $frac{1}{0.015}$ wird gewählt damit die Hauptzahl der Werte zwischen -100 und 100 liegt.
