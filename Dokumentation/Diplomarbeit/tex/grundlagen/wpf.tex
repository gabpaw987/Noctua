...
Zuerst wird hier der DataContext auf das ViewModel gesetzt. Dies wäre hier zwar noch nicht notwendig, da keine Bindung auf ein Property aus dem ViewModel erfolgt, es zeigt allerdings die angesprochene Funktionsweise und wird im nächsten Beispiel von Bedeutung sein. Danach werden zwei TextBoxen erstellt von denen die Text-Variable der unteren eine Bindung auf die Text-Variable der oberen besitzt. Dadurch wird der Text der unteren TextBox automatisch an den Text der oberen TextBox angepasst, falls sich dieser ändert. Eine Bindung in dieser Art könnte aber auch auf jedes andere Attribute der TextBoxen (z.B. Größe, Schriftart, etc.) angewendet werden. Dazu müsste lediglich der Path der Bindung geändert werden. \cite{visualcsharp}\\
\\
...
Diesmal ist die Definition des ViewModels als DataContext notwendig. Aus diesem wird nämlich einfach in der Bindung das FavoriteColor-Property aufgerufen. Schon steht zu jeder Zeit die aktuelle Lieblingsfarbe aus dem ViewModel in der TextBox.