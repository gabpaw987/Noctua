...
Hierbei werden zuerst die nötigen von Microsoft definierten \gls{xaml}-Namespaces angegeben. Anschließend werden ein Titel (der ganz oben im Fenster angezeigt wird) und die Größe des Fensters angegeben, das später unseren Button beinhalten soll. Danach wird ein einfaches Grid-Layout mit einem Button mit dem Text "Button" gezeichnet. Ab Microsoft Visual Studio 2008 wird auch ein \gls{wpf}-\gls{gui}-Builder bei der Installation mitgegeben, mit dem man diese \gls{xaml}-Dateien einfach mit graphischer Oberfläche erstellen kann. \cite{visualcsharp}\\
\\
Zu der MainWindow.xaml-Datei wird auch noch eine weitere Datei mit dem Namen \inline{MainWindow.xaml.cs} erstellt. Diese wird auch als \inline{Code-Behind}-Datei bezeichnet und beinhaltet den Teil der Programmlogik der direkt mit der graphischen Oberfläche verknüpft ist. Möchte man also bspw. die Logik implementieren, die beschreibt, was bei einem Klick auf den Button passiert, müsste man dies in eben dieser MainWindow.xaml.cs-Datei durchführen. \cite{visualcsharp}
...
\begin{itemize}
	...
	\item Collections
	\item Datenbanken
	\item etc. \cite{visualcsharp}
\end{itemize}

Zur Nutzung von Data Binding sind nun zwei Klassen wichtig: \inline{DataContext} und \inline{Binding}. Der \inline{DataContext} ist die Datenquelle, von der die Daten bezogen werden sollen. Dieser \inline{DataContext} ist außerdem ein Attribut jedes \gls{gui}-Elements und muss gesetzt werden, damit Bindungen funktionieren. Möchte man nun den \inline{DataContext} setzen, sollte man aber aufpassen und ihn im übergeordneten Container (z.B. dem \inline{MainWindow}) setzen, denn davon profitieren alle untergeordneten Komponenten dieses Containers und der \inline{DataContext} muss nicht überall gesetzt werden. Das \inline{Binding}-Objekt beschreibt nun die Bindung zwischen einer Datenquelle und einer bindenden Komponente. Das \inline{Binding} wird meist im \gls{xaml}-Code geschrieben, es besteht allerdings selbstverständlich auch die Möglichkeit, sie innerhalb des C\#-Codes zu erzeugen. \cite{visualcsharp}\\
\\
...
Zuerst wird hier der \inline{DataContext} auf das ViewModel gesetzt. Dies wäre hier zwar noch nicht notwendig, da keine Bindung auf ein Property aus dem ViewModel erfolgt, es zeigt allerdings die angesprochene Funktionsweise und wird im nächsten Beispiel von Bedeutung sein. Danach werden zwei \inline{TextBox}en erstellt, von denen die Text-Variable der unteren eine Bindung auf die Text-Variable der oberen besitzt. Dadurch wird der Text der unteren \inline{TextBox} automatisch an den Text der oberen \inline{TextBox} angepasst, falls sich dieser ändert. Eine Bindung in dieser Art könnte aber auch auf jedes andere Attribut der \inline{TextBox}en (z.B. Größe, Schriftart, etc.) angewendet werden. Dazu müsste lediglich der \inline{Path} der Bindung geändert werden. \cite{visualcsharp}\\
\\
...
Diesmal ist die Definition des ViewModels als \inline{DataContext} notwendig. Aus diesem wird nämlich einfach in der Bindung das \inline{FavoriteColor}-Property aufgerufen. Schon steht zu jeder Zeit die aktuelle Lieblingsfarbe aus dem ViewModel in der \inline{TextBox}.