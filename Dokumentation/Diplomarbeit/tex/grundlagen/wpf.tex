...
\\
Zu der MainWindow.xaml-Datei wird auch noch eine weitere Datei mit dem Namen \textit{MainWindow.xaml.cs} erstellt. Diese wird auch als \textit{Code-Behind}-Datei bezeichnet und beinhaltet den Teil der Programmlogik der direkt mit der graphischen Oberfläche verknüpft ist. Möchte man also bspw. die Logik implementieren, die beschreibt, was bei einem Klick auf den Button passiert, müsste man dies in eben dieser MainWindow.xaml.cs-Datei durchführen. \cite{visualcsharp}
...
Zur Nutzung von Data Binding sind nun zwei Klassen wichtig: \textit{DataContext} und \textit{Binding}. Der \textit{DataContext} ist die Datenquelle, von der die Daten bezogen werden sollen. Dieser \textit{DataContext} ist außerdem ein Attribut jedes \gls{gui}-Elements und muss gesetzt werden, damit Bindungen funktionieren. Möchte man nun den \textit{DataContext} setzen, sollte man aber aufpassen und ihn im übergeordneten Container (z.B. dem \textit{MainWindow}) setzen, denn davon profitieren alle untergeordneten Komponenten dieses Containers und der \textit{DataContext} muss nicht überall gesetzt werden. Das \textit{Binding}-Objekt beschreibt nun die Bindung zwischen einer Datenquelle und einer bindenden Komponente. Das \textit{Binding} wird meist im \gls{xaml}-Code geschrieben, es besteht allerdings selbstverständlich auch die Möglichkeit, sie innerhalb des C\#-Codes zu erzeugen. \cite{visualcsharp}\\
\\
