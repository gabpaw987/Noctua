...
Die einfachste Möglichkeit, um Trends in Aktienkursen sichtbar zu machen, ist eine Glättung des Preisverlaufs, also eine Art Durchschnittsberechnung.
...
Dabei ist $P^{*}_t$ der geglättete Wert zum Zeitpunkt $t$, $n$ die Anzahl an einbezogenen Werten und $P_{t-i}$ der Preis zum Zeitpunkt $t-i$. \cite{elder_living} \cite{murphy-technische}
...
Den \gls{dema} darf man sich allerdings nicht als \gls{ema} eines bereits berechneten \gls{ema} vorstellen. Das würde zu einer unerwünschten starken Abwertung aktueller Daten führen, wie an den Gewichtungsgraphen in Abbildung \ref{fig:wrong_dema_tema} zu sehen ist. Tatsächlich wird der \gls{dema} aus einer Zusammensetzung aus einem einfachen und einem doppelten \gls{ema} berechnet, wodurch ein neuer \gls{ma} mit weniger Lag als jede der Komponenten entsteht. \cite{mulloy-smoothing}
...
Kaufman bestimmt die Volatilität durch eine \gls{er}, die berechnet wird, indem die absolute Gesamtpreisbewegung, i.e. Preis am Ende der Periode abzüglich des Preises am Beginn, durch die Summe der absoluten Preisbewegungen in einer Periode dividiert wird. Aufgrund der \gls{er} und zwei gegebenen Längen ($\alpha$ zwischen 0 und 1) wird nun ein tatsächlicher \gls{sf} berechnet. Dieser sollte zwischen den beiden gegebenen Werten liegen, was jedoch nur dann immer der Fall ist, wenn in der Berechnung des \gls{sf} $1$, nicht $2$ als Exponent gewählt wird.
...
Dabei ist $SF_S$ der schnelle Smoothing Factor und $SF_L$ der langsame, zwischen denen das Ergebnis $SF$ abhängig von der $ER$ liegen soll.
Der Exponent kann als Abwandlung des Indikators auch verändert werden, beispielsweise auf $1$, wenn der resultierende Durchschnitt genau im angegebenen Bereich liegen soll, oder auch auf $0.75$, falls konservativere Durchschnitte erwünscht sind. \cite{kaufman_systems} \cite{ama_whs}
...
der Geraden liegen, wobei alle Punkte gleich stark gewichtet werden und auch möglichst gleich weit entfernt liegen sollen. Zur Abstandsmessung werden die quadrierten y-Abstände zwischen Gerade und Preis herangezogen, damit größere Abstände stärker gewichtet werden als kleine. Die Gerade wird nun so gelegt, dass diese Summe der quadratischen Abstände möglichst klein ist.
...
Der \gls{adx} ist ein Indikator, der es auf einfache Weise ermöglicht, die Stärke eines Trends zu bestimmen, jedoch nicht, ob es sich um einen Aufwärts- oder Abwärtstrend handelt. Er wurde Ende der Siebziger von Welles Wilder entwickelt, der u.a. auch für den RSI (s. \ref{subsubsection:rsi}) verantwortlich ist. Der \gls{adx} ist ein Indikator, der sich aus der True Range (TR) und den Directional Movement-Indikatoren $DM_+$ und $DM_-$ zusammensetzt. Alle zusammen werden auch als Directional System bezeichnet. \cite{atr_whs} \cite{elder_living}

Der \gls{adx} wird in 5 Schritten berechnet.

\begin{enumerate}
	\item{Directional Movement ($DM_+$, $DM_-$) berechnen\\
		Directional Movement bezeichnet die Spanne, in der sich der Preis in der aktuellen Periode über oder unter der Preisspanne der letzten Periode bewegt hat.
		$DM_+$ ist dabei die Spanne darüber, $DM_-$ jene darunter. Der $DM$ ist dabei immer positiv, es wird also der Betrag der Differenz verwendet. (siehe Abbildung \ref{fig:dm})}
	\item{True Range ($TR$) berechnen\\
		Die True Range soll die tatsächliche Preisbewegung abbilden, also auch Bewegungen zwischen Perioden, bei denen Close- und Open-Preise
		nicht ident sind. Dafür wird für die $TR$ immer der größte der folgenden 3 Werte herangezogen, die sich aufgrund von Preisen des aktuellen und 
	...
	\item{Directional Indicators ($DI_+$, $DI_-$) berechnen\\
		 Die Directional Indicators sind das Verhältnis der Trendbewegung zur True Range, es gilt also:\\
		\begin{equation}
			DI_+ = \frac{DM_+}{TR}
		\end{equation}
		\begin{equation}
			DI_- = \frac{DM_-}{TR}
		\end{equation}}
	\item{Directional Indicators glätten\\
		Die Directional Indicators ($DI_{n+}$ und $DI_{n-}$) erhält man, indem die Directional Indicators ($DI_+$, $DI_-$) mit einem \gls{ma} geglättet werden,
		wofür in der Regel ein \gls{ema} verwendet wird. Die Länge des verwendeten \gls{ma} ist dabei auf die Tradingstrategie anzupassen.
		$DI_{n+}$ und $DI_{n-}$ sind bereits für ein Handelssystem geeignet, da sie das Verhältnis von relativer Trendbewegung zur True Range geglättet angeben.
		Aufgrund der Kreuzung der beiden Indikatoren können so auch Kauf- und Verkauf-Signale gegeben werden.}
	\item{$ADX$ berechnen\\
		Der \gls{adx} ist nun die nochmals geglättete Differenz zwischen $DI_{n+}$ und $DI_{n-}$ und wird wie folgt berechnet:\\
		\begin{equation}
...
Der \gls{adx} ist hilfreich, indem er anzeigt, ob sich der Markt in einer Trendphase befindet, in der Handelssysteme, die auf Trendfolgesystemen basieren, gut funktionieren, oder in einer Seitwärtsphase,
...
\begin{figure}
	...
	\caption[10-Tage Momentum]{10-Tage Momentum, der Überkauft- und Überverkauft-Level anzeigt. \cite{murphy-technische}}
	\label{fig:momentum}
\end{figure}
...
Dabei dienen die Bänder meist als Kursziele, so dass beim Ansteigen des Kurses vom unteren Bollinger-Band das obere als Ziel angenommen wird. Diese Strategie wird auch als "`Fading"' bezeichnet und ist nur in Seitwärtsphasen gewinnbringend, da der Preis während starken Trends lange Zeit entlang dem oberen oder unteren Rand des Bandes verlaufen kann. Die Breite der Bänder zeigt die Volatilität des Kurses an, da bei geringer Volatilität die Standardabweichung ebenfalls sinkt, ergo werden die Bänder schmäler. Verschmälern sich die Bänder in relativ kurzer Zeit, kann meist mit einem Ausbruch und einsetzendem Trend gerechnet werden. Bollinger Bänder können insgesamt sehr vielseitig eingesetzt werden, nicht nur in Seitwärtsphasen, es existieren auch unterschiedliche Interpretations-Varianten.

\begin{figure}
	\centering
		\includegraphics[width=0.8\textwidth]{graphics/fingrundlagen/bollinger.png}
	\caption[20-Tage Bollinger Bänder]{20-Tage Bollinger Bänder mit doppelter Standardabweichung am Beispiel des Dow Jones \cite{murphy-technische}}
	\label{fig:bollinger}
\end{figure}
...
Pivot Points sind eine sehr häufig genutzte Variante zur Berechnung von horizontalen Support- und Resistance-Leveln. Dazu werden jeweils für eine Periode die Preisbewegungen der vorherigen Periode als Berechnungsgrundlage herangezogen. Als erstes wird der Typical Price (TP) der letzten Periode wie folgt berechnet.

\begin{equation}
TP = \frac{High+Low+Close}{3}
\end{equation}

Dieser wird als auch als Haupt-Pivot-Point bezeichnet, da von ihm ausgehend alle Unterstützungs- und Widerstandslevel festgelegt werden. Zur Berechnung verwendet werden immer die Daten aus der letzten Periode. Es werden zwei Level sowohl von Support- ($SL_1$ und $SL_2$) als auch von Resistance-Level ($RL_1$ und $RL_2$) berechnet, die je nach Marktsituation für unterschiedliche Zwecke ausgewählt werden können.
