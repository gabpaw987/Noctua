...
Die Programmiersprache F\# verwendet zur Methodendefinition den Befehl \inline{let}. Jede definierte Funktion ist im gleichen Namespace erreichbar. Prinzipiell ist eine einmal definierte Funktion nicht mehr änderbar, allerdings gibt es eine Ausnahme: es ist möglich durch die Zusatzdefinition \inline{mutable} eine Funktion bereits beim Erzeugen als änderbar zu kennzeichnen. \clearpage
...
\begin{itemize}
	\item \inline{Array.max} gibt den höchsten Wert des Arrays zurück
	\item \inline{Array.min} gibt den niedrigsten Wert des Arrays zurück
	\item \inline{Array.init} erzeugt ein neues Array mit einer übergebenen Funktion
	\item \inline{Array.averaged} oder \inline{Array.averagedBy} berechnet einen Durchschnitt für alle Elemente im Array
	\item \inline{Array.fold} erzeugt ein invertiertes Array aus dem Vorhandenen
	\item \inline{Array.find} überprüft ob das gegebene Element in der Liste zu finden ist
...
\begin{itemize}
	...
	\item \inline{Seq.windowed} oder \inline{Seq.pairwise} geben die Möglichkeit, die Sequenz in eine Menge Subsequenzen zu spalten, also einem Subsequence-Array
	\item \inline{Seq.cache} sorgt dafür, dass schon einmal evaluierte Ergebnisse nicht noch einmal berechnet werden müssen
...
\begin{itemize}
	\item \inline{List.Head} übergibt das erste Dokument
	\item \inline{List.Empty} ist eine statische Funktion, welche immer eine leere Liste mit n Einträgen zurückgibt
	\item \inline{List.IsEmpty} überprüft die Liste, ob sie leer ist
	\item \inline{List.Item} gibt das n-te Element der Liste zurück
	\item \inline{List.Length} zeigt die Länge der Liste
	\item \inline{List.Tail} gibt die Liste ohne de ersten Eintrag zurück
\end{itemize}
