...
F-Sharp (Visual F\# oder auch F\#) ist eine, im Grunde gesehen, funktionale Programmiersprache, welche auf große Datenmengen und schnelle Berechnungen optimiert ist. Die mögliche Interoperabilität mit allen .NET-Sprachen macht die Sprache zu einer idealen Komponente eines Softwareprodukts. F\# ist konzipiert, um auf eine große Menge an Daten zu wirken, dies wird mittels zusätzlichen Methoden für Listen, Sequenzen und Arrays möglich gemacht. \cite{fsharp}  \\ \\
...
Durch die Anbindung an das .NET-Framework wird F\# allerdings zu einer Multi-Paradigmen-Sprache. Es wird ermöglicht, funktional, objektorientiert, aber auch imperativ arbeiten zu können. \cite{fsharp}\\ \\
\begin{figure}[h]
	\centering
...
Die Programmiersprache F\# verwendet zur Methodendefinition den Befehl \textit{let}. Jede definierte Funktion ist im gleichen Namespace erreichbar. Prinzipiell ist eine einmal definierte Funktion nicht mehr änderbar, allerdings gibt es eine Ausnahme: es ist möglich durch die Zusatzdefinition \textit{mutable} eine Funktion bereits beim Erzeugen als änderbar zu kennzeichnen. \clearpage
...
Arrays sind in F\# veränderbare Sammlungen von längenmäßig fixierten gleichen Datentypen. Sie werden auf folgende Art und Weise definiert:
...
\end{lstlisting}
Für mathematische Berechnungen besitzen die F\#-Arrays zusätzliche Funktionen.
\begin{itemize}
	\item \textit{Array.max} gibt den höchsten Wert des Arrays zurück
	\item \textit{Array.min} gibt den niedrigsten Wert des Arrays zurück
	\item \textit{Array.init} erzeugt ein neues Array mit einer übergebenen Funktion
	\item \textit{Array.averaged} oder \textit{Array.averagedBy} berechnet einen Durchschnitt für alle Elemente im Array
	\item \textit{Array.fold} erzeugt ein invertiertes Array aus dem Vorhandenen
	\item \textit{Array.find} überprüft ob das gegebene Element in der Liste zu finden ist
	\item \textit{Array.toSeq} wandelt das Array in eine Sequenz um 
	\item \textit{Array.toList} wandelt das Array in eine Liste um
\end{itemize}
Ein weiterer Vorteil von Arrays in F\# ist die Möglichkeit, spezifische Einträge einzeln anzusprechen, also einen einzelnen Wert ohne ein Durchsuchen des Arrays zu finden. \cite{arraysfs}
\subsection{Sequenzen in F\#} \label{fsharpsequenzen}
Sequenzen in F\# sind Aneinanderreihungen von Daten des gleichen Daten\-typs mit einer fixierten Größe. Der Vorteil gegenüber Listen in F\# besteht in der Art der Auswertung. Während Listen nur mit evaluierten Werten befüllt werden, also mit Ergebnissen aus den Funktionen, wird eine Sequenz prinzipiell nur mit einer Menge an Anweisungen befüllt. Deswegen ist die Art der Mengenspeicherung am Besten für Datenmengen geeignet, die möglicherweise nicht vollständig gelesen wird. \cite{seqfs} 
\begin{lstlisting}[label=Sequenzen erzeugen,caption=Sequenzen erzeugen]
let sequenz = seq{1 .. 4}
\end{lstlisting}
An diesem Beispiel erkennt man, wie man einer Sequenz eine Anweisung gibt, in diesem Fall:"`Zähle von 1 bis 4"'. In der Sequenz ist dieser Befehl gespeichert, wird allerdings erst dann ausgeführt, wenn auf die Daten zugegriffen werden soll. \cite{seqfs} 
...
\begin{itemize}
	\item \textit{Seq.sort} oder \textit{Seq.sortBy} sortiert die Sequenz 
	\item \textit{Seq.windowed} oder \textit{Seq.pairwise} geben die Möglichkeit, die Sequenz in eine Menge Subsequenzen zu spalten, also einem Subsequence-Array
	\item \textit{Seq.cache} sorgt dafür, dass schon einmal evaluierte Ergebnisse nicht noch einmal berechnet werden müssen
	\item \textit{Seq.take} oder \textit{Seq.truncate} erzeugt aus einer vorhandenen Sequenz eine Subsequenz, also einen Teil der vorhandenen Sequenz
	\item \textit{Seq.toArray} wandelt die Sequenz in ein Array um 
	\item \textit{Seq.toList} wandelt die Sequenz in eine Liste um
\end{itemize}
\clearpage
\subsection{Listen in F\#} \label{fsharplisten}
F\# unterstützt allerdings auch dynamischere Datenmengen, Listen, welche nicht durch die Länge eingeschränkt wird. Im Vergleich zu Arrays sind die gespeicherten Werte in den Listen nicht mehr änderbar. Das Problem, welches durch die Verwendung von F\#-Listen aufkommt, ist, dass bei einem Zugriff auf einen Eintrag der Liste, jedes einzelne Mal die gesamte Liste durchgearbeitet werden muss, bis der gewünschte Eintrag gefunden wurde. \cite{listfs}
\begin{lstlisting}[label=Erzeugen einer Liste,caption=Erzeugen einer Liste]
let list = [ 1; 2; 3 ]
\end{lstlisting}
In F\# ist es möglich, solche Listen innerhalb einer Zeile dynamisch zu erzeugen. 
...
\end{lstlisting}
F\# bietet folgende Eigenschaften, um mit Listen arbeiten zu können:
\begin{itemize}
	\item \textit{List.Head} übergibt das erste Dokument
	\item \textit{List.Empty} ist eine statische Funktion, welche immer eine leere Liste mit n Einträgen zurückgibt
	\item \textit{List.IsEmpty} überprüft die Liste, ob sie leer ist
	\item \textit{List.Item} gibt das n-te Element der Liste zurück
	\item \textit{List.Length} zeigt die Länge der Liste
	\item \textit{List.Tail} gibt die Liste ohne de ersten Eintrag zurück
\end{itemize}
Listen eignen sich sehr gut für rekursive Funktionen, weil durch die Prozeduren List.Head und List.Tail das im ersten Absatz dieses Kapitels genannter Problem nicht auftreten kann. \cite{listfs}
\clearpage
\subsection{Tupel in F\#}
Ein Tupel ist eine bestimmte Menge an unterschiedlichen geordneten Werten in F\#. Solche Tupel haben eine feste Größe und die Werte in einer solchen Menge müssen bestimmte Datentypen besitzen und dürfen von diesen nicht abweichen. Werte, die in Tupeln gespeichert sind, können nicht verändert werden, das bedeutet, dass keine mutable (veränderbaren) Werte gespeichert werden können. \cite{tupelfs}
\begin{lstlisting}[label=Erzeugen einer Tupel,caption=Erzeugen einer Tupel]
let tupel = ("Hello", 17m, true, 'a', 0.01, 1)
\end{lstlisting}
An dem Beispiel \ref{Erzeugen einer Tupel} erkennt man, dass einer Tupel unterschiedliche Datentypen verwenden kann. \\ \\ Eine Tupel wird prinzipiell in runden Klammern () definiert. Man kann auch in der Funktionsdefinition beschreiben wie der Tupel aussehen soll. \cite{tupelfs}
\begin{lstlisting}[label=Vordefinition von Tupeln,caption=Vordefinition von Tupeln]
let f:(string * decimal * bool * char * float * int)= ...
\end{lstlisting}
Um auf die Werte eines Tupel zugreifen zu können, muss man zusätzliche Funktionen implementieren:
...
\end{lstlisting}
Bei Algorithmen, die Werte wie Datum, Uhrzeit, Open (Dezimalzahl), High, Low und Close ist der grundlegende Vorteil eines Tupels klar deutlich.\\ In solchen Tupeln können die Datenstrukturen perfekt übernommen werden, und man kann sich auf eine Datenmenge konzentrieren, ohne andauernd mehrere Arrays oder Listen durchzurechnen, um die jeweils passenden Daten zu erhalten. \\
Es gibt bereits definierte Funktionen um Tupelzugriffe auf das erste und zweite Element eines Tupels durchzuführen. Allerdings funktionieren diese vordefinierten Funktionen nur bei Tupeln die 2 Einträge besitzen. \\ \\
Tuple bieten also die Funktionalität, zusammengehörende Daten vernünftig zu speichern. Dieser Effekt bietet einige Vorteile, jedoch gibt es nur wenige vordefinierten Funktionen, um auf die eingeschlossenen Daten zu wirken. \cite{tupelfs}
\clearpage
\subsection{Pattern Matching in F\#}
In F\# gibt es eine Möglichkeit, Entscheidungen mittels Pattern Matching zu verwirklichen. Diese Funktionalität der Programmiersprache ist vergleichbar mit der Switch-Case-Anweisung aus bekannteren Sprachen wie JAVA oder C\#. Mit Pattern Matching können Datenmengen mittels einer Entscheidungslogik bearbeitet werden. \\ \\
Eine Beispielanwendung wäre ein vereinfachter Signalgeber. Bei einer Signalspitze soll das Programm eine Ausgabe schreiben. \cite{pmfs}
...
In F\# existiert ein zusätzlicher Parameter, um Ergebnisse von Funktionen in weitere Funktionen zu übergeben. Der Zweck dieses Operators ist, dass man nicht zu viele Zwischenspeichervariablen und Funktionen definieren muss. Das Prinzip hinter diesem Operator ist sehr simpel, das Ergebnis aus dem ersten Funktionsaufruf wird als letzter Übergabeparameter der nächsten Funktion übergeben.
...
Sehr oft macht es keinen Sinn Zwischenergebnisse zu speichern, weil diese nur einmal verwendet werden müssen. Deswegen sollte man, wenn man mit F\# arbeitet, meistens den Pipeoperator benutzen, außer es ist explizit nicht notwendig, weil man das evaluierte Ergebnis noch einmal benötigt. \cite{sofs}
