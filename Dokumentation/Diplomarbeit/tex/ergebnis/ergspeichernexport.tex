...
\\
Beim Speichern werden alle Einstellungen des Settings-Tabs sowie alle allgemeinen und Order-spezifischen Performancedaten binär serialisiert (siehe \ref{Binaere-Serialisierung} "`Binäre Serialisierung"') und in einem speziellen .bts-File gespeichert. Dieses kann zu einem späteren Zeitpunkt auch wieder geladen werden, wobei der komplette Zustand der \gls{bts} bis auf das gezeichnete Chart wiederhergestellt wird. Dies funktioniert vor allem recht einfach, da bei jedem Speicher- sowie Lese-Vorgang die serialisierten Daten in derselben Reihenfolge gespeichert bzw. ausgelesen werden können. Wie die \gls{bts} nach dem Laden ohne Chart aussieht, kann der Abbildung \ref{fig:bts_order_geladen} entnommen werden.\\
...
Sowohl das Speichern und Laden als auch das Exportieren benützen spezielle FileDialogs (\inline{OpenFileDialog} und \inline{SaveFileDialog}) zur Auswahl der zu ladenden Datei bzw. des gewünschten Speicherorts der Datei. Diese FileDialog sieht bspw. beim Speichern eines .bts-Files aus wie in Abbildung \ref{fig:bts_speichern}.\\
