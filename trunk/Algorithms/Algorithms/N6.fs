namespace Algorithm
    module DecisionCalculator=(*124*)
        ...
        *)
        ...
        (*
         * returns the the absolute maximum (w.o. sign) for the last n bars
         * for each element
         *)
        ...
        let strategy(erp:int,
            ...
            let rsiRegrN = 10
            let rsiRegr = 
                [|for i in rsiRegrN+rsiN - 2..rsi.Length-1 -> lwr(2, rsi.[i-rsiRegrN+1..i])|]
            ...
            // 60 bar long WEIGHTED regression of prices
            let regrLN = 60
            let regrL =
                [|for i in regrLN-1..cPrices.Length-1 -> lwr(4, cPrices.[i-regrLN+1..i])|]
            ...
            let slopeM = [| for i in mExtrema (20, regrM) -> i*100m |]
            //for i in 0..slopeM.Length-1 do chart2.["slopeM;#FF0000"].Add(slopeM.[i])
            ...
            // last significant stark price movement
            let mutable starkMove = 0
            // duration since last stark movement
            let mutable starkMoveFor = 0
            ...
            let mutable priceSig = 0
            let mutable exit = false
            let mutable tryExit = false
            // do not revert signal
            let mutable lockSignal = false
...
//                    amaSig <- (sign amaSig) * -1
                ...
                // add to chart2
                //chart2.["relBarSize;#00FF00"].Add(relBarSize)

                if i <> 0 then
                    // increment starkMove duration counter
                    if (starkMove <> 0) then
                        starkMoveFor <- starkMoveFor + 1
                        // reset after certain period
                        if (starkMoveFor > 8) then
                            starkMove <- 0

                    // check if current bar is a stark move
                    if (relBarSize > 2.0m) then
                        starkMoveFor <- 0
                        if (fst ocPrices.[i] < snd ocPrices.[i]) then
                            starkMove <- 1
                        else
                            starkMove <- -1

                    // significant down movement after prices starkly moved up
                    if (starkMove = 1 && fst ocPrices.[i] > snd ocPrices.[i] && relBarSize > 1m) then
                        priceSig <- -1
                        starkMove <- 0
                    // significant up movement after prices starkly moved down
                    else if (starkMove = -1 && fst ocPrices.[i] < snd ocPrices.[i] && relBarSize > 1m) then
                        priceSig <-  1
                        starkMove <- 0
                    // sit out extreme movements
                    else if (relBarSize > 1.5m) && ((fst ocPrices.[i] < snd ocPrices.[i] && signals.[i-1] > 0) || (fst ocPrices.[i] > snd ocPrices.[i] && signals.[i-1] < 0)) then
                        priceSig <- signals.[i-1]
                    else
                        priceSig <- 0
                    ...
                    if (signals.[i-1] <> 0) then
                        ...
                        if (sign signals.[i-1] = 1) then
                            ...
                            if (fst ocPrices.[i] > snd ocPrices.[i]) && (rsi.[i] > 70m && cPrices.[i] > entryPrice && relBarSize > 0.8m) || (cPrices.[i]-entryPrice > 0.8m) then
                                exit <- true
                            // loss with short regression downwards
                            if (cPrices.[i] < entryPrice && regrS.[i] < 0m) then
                                tryExit <- true
                        // short
                            ...
                            if (fst ocPrices.[i] < snd ocPrices.[i]) && (rsi.[i] < 30m && cPrices.[i] < entryPrice && relBarSize > 0.8m) || (cPrices.[i]-entryPrice < -0.8m) then
                                exit <- true
                            // loss with short regression upwards
                            if (cPrices.[i] > entryPrice && regrS.[i] > 0m) then
                                tryExit <- true

                        // exit from long if profitable and red bar
                        if (sign signals.[i-1] = 1) && (tryExit) && (cPrices.[i] - entryPrice > 0m) && (fst ocPrices.[i] > snd ocPrices.[i]) then
                            exit <- true
                        else if (sign signals.[i-1] = 1) && (tryExit) && (cPrices.[i] - entryPrice < 0m) && (fst ocPrices.[i] < snd ocPrices.[i]) then
                            exit <- true
                    else
                        tryExit <- false
...
//                    else if false then
                        ...
                        if (rsiSig <> 0 && sign rsiSig <> sign amaSig) then
                            //signals.Add(0)
                            ...
                            if (amaSig = 2 && regrS.[i] > 0m && regrM.[i] > 0m) || (amaSig = -2 && regrS.[i] < 0m && regrM.[i] < 0m)  then
                                ...
                                signals.Add(
                                    // short and medium term trend coincide
                        ...
                        // new signal that should not be executed
                        if (lockSignal && sign signals.[i] <> sign signals.[i-1]) then
                            // keep old signal
                            signals.[i] <- signals.[i-1]
                        if (signals.[i] = 0 || sign signals.[i] = sign signals.[i-1]) then
                            lockSignal <- false

                        // overrule with price signals
                        if (priceSig <> 0) then
                            signals.[i] <- priceSig
                            lockSignal <- true

                        // do not decide on one single stark move
                        if (abs starkMove = 1 && starkMoveFor = 0 && signals.[i] <> signals.[i-1]) then
                            signals.[i] <- signals.[i-1]
...
//                        if (signals.[i-1] <> 0) then
//                            for j in 2..10 do
//                                if (sign signals.[i-j] <> sign signals.[i-j+1]) then
//                                    signals.[i] <- signals.[i-1]

                        if (exit) then
                            signals.[i] <- 0
                            // wait 10 bars until deciding anew
                            missingData <- 10
...
//                            signals.[i] <- signals.[i-1]
        ...
        let startCalculation (prices:System.Collections.Generic.List<System.Tuple<System.DateTime, decimal, decimal, decimal, decimal>>, 
            ...
            chart2.Add("slopeM;#FF0000", new System.Collections.Generic.List<decimal>())
            chart2.Add("relBarSize;#00FF00", new System.Collections.Generic.List<decimal>())
