...
namespace Algorithm
    module DecisionCalculator89352=(*018*)
...
//        let mutable ns = new System.Collections.Generic.List<decimal>()
        ...
        let startCalculation (prices:System.Collections.Generic.List<System.Tuple<System.DateTime, decimal, decimal, decimal, decimal>>, 
            ...
            *)
            ...
            let mutable takeEarnings = takeEarningsMax
            // max loss
            let maxLoss = parameters.["maxLoss"]
            ...
            // value for current gain/loss
            let mutable gainLoss = 0m
            let mutable lastEntryPrice = 0m
...
//                    chart2.["LocalExtremes;#00FFFF"].Add(localExtrema.[localExtrema.Length-1])
                    ...
                    if (takeEarningsMax <> 0m) then
                        ...
                        // same trading direction (-/+)
                            ...
                            // check daily loss
                            if (maxLoss <> 0m) then
                                let currentLoss = ((entryPrice - cPrices.[i]) * decimal (sign signals.[i]))
                                if (gainLoss + currentLoss < -maxLoss ) then
                                    exit <- 0
                                    gainLoss <- - maxLoss

                    (*
                     * Check price difference if signal has changed for max loss per day
                     *)
                    if (maxLoss <> 0m) then
                        if (i <> 0) then
                            if (signals.[i] <> signals.[i-1] && signals.[i-1] <> 0) then 
                                let ab = ((lastEntryPrice - cPrices.[i]) * decimal (sign signals.[i]))
                                gainLoss <- gainLoss + ab
                            if (prices.[i].Item1.DayOfWeek <> prices.[i - 1].Item1.DayOfWeek) then
                                gainLoss <- 0m

                        (*
                         * Liquidate if daily limit is reached
                         *)
                        if (gainLoss <=  -maxLoss) then 
                            exit <- 0
